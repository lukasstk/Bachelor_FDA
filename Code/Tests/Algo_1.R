...
# ---- 6) Daten aufbereiten für Plot ----

# 1. df_full: Originaldaten (alle Kurven, unzensiert)
df_full <- as.data.frame(bm_mat) %>%
  mutate(id = rownames(bm_mat),
         version = "vollständig",
         gruppe = NA) %>%  # links immer grau (über na.value geregelt)
  pivot_longer(cols = -c(id, version, gruppe), names_to = "t_idx", values_to = "x") %>%
  mutate(t = grid_plot[as.integer(sub("V", "", t_idx))])

# 2. df_obs: Beobachtete Werte (mit NAs) + Gruppenzuweisung
df_obs <- as.data.frame(X_obs) %>%
  mutate(id = rownames(X_obs),
         version = "beobachtet (mit Lücken)",
         gruppe = ifelse(rowSums(is.na(.)) == 0, "vollständig", "unvollständig")) %>%
  pivot_longer(cols = -c(id, version, gruppe), names_to = "t_idx", values_to = "x") %>%
  mutate(t = grid_plot[as.integer(sub("V", "", t_idx))])

# 3. Combine für Plot
df_plot <- bind_rows(df_full, df_obs) %>%
  mutate(
    version = factor(version, levels = c("vollständig", "beobachtet (mit Lücken)")),
    gruppe = factor(gruppe, levels = c("vollständig", "unvollständig"))
  )

# ---- 7) Plot ----
ggplot(df_plot, aes(x = t, y = x, group = id)) +
  geom_line(aes(color = gruppe), size = 0.9, na.rm = TRUE) +
  facet_grid(. ~ version) +
  scale_color_manual(
    values = c("vollständig" = "grey", "unvollständig" = "black"),
    na.value = "darkgray"  # linke Seite
  ) +
  labs(
    title = "Brownian-Pfade: Links vollständig, Rechts MNAR-Zensur",
    x = "Zeit t", y = "X(t)", color = "Gruppe"
  ) +
  theme_minimal() +
