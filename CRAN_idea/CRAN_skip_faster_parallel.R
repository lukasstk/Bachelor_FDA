...
#' @param progress Fortschrittsbalken über Chunks anzeigen?
#' @param progress_style Style für `txtProgressBar` (1–3).
...
boot_mean_test <- function(fd = NULL, X_obs = NULL, groups = NULL, observed_ratio = 1,
  ...
  ## ---- Backend-Management: auto / force_pool / sequential -------------------
  ...
  if (!isTRUE(parallel) || identical(manage_backend, "sequential")) {
    ...
    if (identical(manage_backend, "auto") && has_external_backend) {
      ...
      # BLAS/OpenMP nur auf Workern drosseln (keine globalen Seiteneffekte)
  ...
  ## ---- Progressbar via Combiner ---------------------------------------------
  pb <- NULL
  if (isTRUE(progress)) {
    pb <- txtProgressBar(min = 0, max = n_chunks, style = progress_style)
    chunks_done <- 0L
    comb <- function(acc, val) {
      chunks_done <<- chunks_done + 1L
      setTxtProgressBar(pb, chunks_done)
      if (is.null(acc)) val else rbind(acc, val)
    }
  } else {
    comb <- function(acc, val) if (is.null(acc)) val else rbind(acc, val)
  }
